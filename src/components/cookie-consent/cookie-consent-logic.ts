...
function CCM_setCookies(isTracking: boolean) {
    ...
    if (isTracking) {
        document.cookie = "ccm_trackAnalytics=yes; path=/; expires=" + expires.toUTCString() + "; Secure";
    } else {
        document.cookie = "ccm_trackAnalytics=no; path=/; expires=" + expires.toUTCString() + "; Secure";
    }
    // save that the popup has been seen and accepted
    document.cookie = "ccm_accepted_v" + cookieConsentVersion + "=yes; path=/; expires=" + expires.toUTCString() + "; Secure";
...
export function enableTracking() {
    ...
    console.log('gtag enable tracking')
    gtag('consent', 'update', {
...
export function showCookiebanner(): boolean {
    return CCM_getCookie("ccm_trackAnalytics")?.length === 0 || CCM_getCookie("ccm_trackAnalytics") === "no";
}
